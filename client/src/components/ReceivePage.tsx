...
export default function ReceivePage({ initialCode }: ReceivePageProps) {
  const { status, fileMeta, stats, error, receivedFile, joinRoom, acceptTransfer, reset } = useReceiver();
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {/* Prompting state */}
      {status === "prompting" && fileMeta && (
        <div className="animate-slide-up panel p-8 text-center space-y-6">
          ...
          <div>
            <p className="font-display text-white text-xl">Incoming Transfer</p>
            <p className="text-muted text-sm mt-1">
              Sender wants to send <strong>{fileMeta.name}</strong> ({formatBytes(fileMeta.size)})
            </p>
          </div>
          <button className="btn-primary w-full text-lg" onClick={acceptTransfer}>
            Accept & Download
          </button>
        </div>
      )}
        ...
        <div className="animate-slide-up space-y-4">
          <div className="panel p-8 text-center space-y-4">
            ...
            <div>
              ...
              {receivedFile.url === "" && (
                <p className="text-success text-xs mt-2 font-mono bg-success/10 py-1 px-2 rounded inline-block">
                  Saved directly to your disk
                </p>
              )}
            </div>
          </div>
